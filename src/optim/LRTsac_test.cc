...
BOOST_AUTO_TEST_CASE(TestOptions) {
  LRToptions options;
  BOOST_CHECK_EQUAL(options.c, 25);
  BOOST_CHECK_EQUAL(options.beta, 0.05);
  BOOST_CHECK_EQUAL(options.dim, 1);
  ...
  BOOST_CHECK_EQUAL(options.B, 20);
}

BOOST_AUTO_TEST_CASE(TestReport) {
  LRTsac<SimilarityTransformEstimator<3>>::Report report;
  BOOST_CHECK_EQUAL(report.success, false);
  BOOST_CHECK_EQUAL(report.num_trials, 0);
  BOOST_CHECK_EQUAL(report.vpm, 0);
...
BOOST_AUTO_TEST_CASE(TestSimilarityTransform) {
  ...
  LRToptions options;
  options.sigma_min = 0.5;
  options.sigma_max = 15;
  options.delta_sigma = 0.5;
  options.dim = 3;
  options.D = 1000;

  LRTsac<SimilarityTransformEstimator<3>> lrtsac(options);
